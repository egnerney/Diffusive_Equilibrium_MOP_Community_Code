...
Diffusive Equilibrium Code with Phase Space Hole (PSH) support
...
PSH version handles cases where reference location s0 and target location s
are on opposite sides of a local maximum in deltaU (choke point).
...
Test Example use of Diffusive Equilibrium MOP community code with PSH support
"""


from diffusive_equilibrium_psh import *
...
if __name__ == '__main__':
    ...
    species_0 = []
    # Fill Species class with plasma parameters at s_0 defined above for use with diff_eq_psh code
    ...
    # Loop over each point to compute densities along field line using PSH version
    for i in range(npointsfl):
        ...
        result = diff_eq_psh(
            Pos[i, :],
            Pos0,
            lat[i],          # lat at current position
            lat[ceq_idx][0], # lat at reference position
            Bratio[i],
            species_0,
            deltaU[i],
            deltaU,          # full deltaU array
            lat,             # full lat array
            B,               # full B array
            Bratio,          # full Bratio array
            planet,
            fcor)
    ...
    plt.legend(fontsize='x-small', ncol=2)
    plt.grid()
    plt.savefig(species_type[0] + "_diff_all_eq_densities_vs_lat_log_aligned_Io_fl_psh.png", dpi=600, bbox_inches="tight")
    plt.show()
    ...
    plt.legend(fontsize='x-small', ncol=2)
    plt.grid()
    plt.savefig(species_type[0] + "_all_diff_eq_kappa_perp_vs_lat_log_aligned_Io_fl_psh.png", dpi=600, bbox_inches="tight")
    plt.show()
    ...
    """
    # Save dictionary of numpy arrays of plasma paramter outputs with PSH
    ...
    np.savez_compressed(species_type[0] + "Diff_eq_plasma_parameter_along_Io_aligned_fl_JRM33+Con2020_psh.npz',
    ...
    with np.load('Diff_eq_plasma_parameter_along_Io_aligned_fl_JRM33+Con2020_psh.npz', allow_pickle=True) as data:
        n = data['n'].item()

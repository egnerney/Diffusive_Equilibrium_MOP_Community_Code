...
CPU Parallelized Version using multiprocessing pool
"""


from diffusive_equilibrium_cpu_parallelized import *
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*60}")
    print(f"Starting optimized parallel computation with {npointsfl} points...")
    print(f"Distribution type: {species_type[0]}")
    print(f"{'='*60}\n")
    
    start_time = time.time()
    
    # Use the parallel version for computation
    n_out, phi, Tpar_out, Tperp_out, kappapar_out, kappaperp_out = diff_eq_parallel(
        Pos, Pos0, Bratio, species_0, deltaU, planet, fcor,
        n_workers=None,  # Use all available CPUs
        chunk_size=50    # Process 50 points per chunk
    )
    ...
    print(f"Points per second: {npointsfl/computation_time:.1f}")
    print(f"{'='*60}\n")
    
    # Original non-parallel version for comparison (commented out)
    """

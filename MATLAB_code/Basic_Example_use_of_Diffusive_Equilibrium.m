...
% ----------------------------------------------------------------------------
% A line-for-line MATLAB translation of your Diffusive Equilibrium example,
% now using renamed species to avoid invalid struct field names. 
%
% Requirements:
...
%% Fundamental constants matching your Python code
...
% Here are your newly revised species names to avoid +/-
species_names = {
    'Op',   ...  % was 'O+'
    'O2p',  ...  % was 'O++'
    'Sp',   ...  % was 'S+'
...
% The .mat "species" should have been updated in the same order:
%   O+ -> Op, O++ -> O2p, etc. 
% We'll just trust the order is correct and skip the check.
...
% Let's assume your older code matches these lines up with the new naming
species_m = [
    m_Op,  ...  % 'Op' 
    m_O2p, ...  % 'O2p'
    m_Sp,  ...  % 'Sp'
...
%dist_type = repmat({'Maxwellian'}, 1, nspec);
%dist_type = repmat({'Aniso_Maxwellian'}, 1, nspec);
%dist_type = repmat({'Aniso_Maxwellian_const_Tperp'}, 1, nspec);
%dist_type = repmat({'Aniso_kappa'}, 1, nspec);
%dist_type = repmat({'Aniso_product_kappa'}, 1, nspec);
dist_type = repmat({'Fried_Egg'}, 1, nspec);
...
set(gca,'YScale','log');
title([dist_type{1} ' Diffusive Equilibrium, Aligned Io Field Line']);

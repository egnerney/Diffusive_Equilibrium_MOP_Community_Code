...
!  Input:
!
!    real ( kind = rk8 ) X, the argument of the Airy function.
!
!  Output:
!
!    real ( kind = rk8 ) AI, BI, AD, BD, the values of Ai(x), Bi(x),
!    Ai'(x), Bi'(x).
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, argument of Airy function.
!
!    Output, real ( kind = rk8 ) AI, Ai(x).
!
!    Output, real ( kind = rk8 ) BI, Bi(x).
!
!    Output, real ( kind = rk8 ) AD, Ai'(x).
!
!    Output, real ( kind = rk8 ) BD, Bi'(x).
!
  implicit none
...
!    2 for Bi(x) and Bi'(x).
!
!    Output, real ( kind = rk8 ) XA(m), a, the m-th zero of Ai(x) or
!    b, the m-th zero of Bi(x).
!
!    Output, real ( kind = rk8 ) XB(m), a', the m-th zero of Ai'(x) or
!    b', the m-th zero of Bi'(x).
!
!    Output, real ( kind = rk8 ) XC(m), Ai(a') or Bi(b').
!
!    Output, real ( kind = rk8 ) XD(m), Ai'(a) or Bi'(b)
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, the argument.  X should not be zero.
!
!    Output, real ( kind = rk8 ) VJ1, VJ2, VY1, VY2, VI1, VI2, VK1, VK2,
...
!
!    Input, real ( kind = rk8 ) C, the spheroidal parameter.
!
!    Input, real ( kind = rk8 ) X, the argument of the angular function.
!    |X| < 1.0.
!
...
!
!    Input, real ( kind = rk8 ) CV, the characteristic value.
!
!    Output, real ( kind = rk8 ) S1F, S1D, the angular function of the first
...
!
  ...
  real ( kind = rk8 ) a0
  real ( kind = rk8 ) c
  real ( kind = rk8 ) ck(200)
  real ( kind = rk8 ) cv
  real ( kind = rk8 ) d0
  real ( kind = rk8 ) d1
  real ( kind = rk8 ) df(200)
  real ( kind = rk8 ) eps
  integer ip
  integer k
  integer kd
...
!    Input, real ( kind = rk8 ) C, the spheroidal parameter.
!
!    Input, real ( kind = rk8 ) X, the argument, with |X| < 1.0.
!
!    Input, integer KD, the function code.
!    1, the prolate function.
!    -1, the oblate function.
...
!
  ...
  integer nm
  integer nm2
  real ( kind = rk8 ) pd(0:251)
  real ( kind = rk8 ) pm(0:251)
  real ( kind = rk8 ) s1d
  real ( kind = rk8 ) s1f
  real ( kind = rk8 ) su1
  real ( kind = rk8 ) sw
...
!
!    Output, real ( kind = rk8 ) BN, the value of the N-th Bernoulli number.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) P, Q, the parameters.
!    0 < P, 0 < Q.
!
!    Output, real ( kind = rk8 ) BT, the value of B(P,Q).
!
  implicit none
...
!
!    Output, real ( kind = rk8 ) BJ(N+1), DJ(N+1), FJ(N+1), the values of
...
!
  ...
  real ( kind = rk8 ) bj(n+1)
  real ( kind = rk8 ) bs
  real ( kind = rk8 ) dj(n+1)
  real ( kind = rk8 ) f
  real ( kind = rk8 ) f0
  real ( kind = rk8 ) f1
  real ( kind = rk8 ) fj(n+1)
  integer k
  integer m
  integer mt
  integer nt
...
!    Input, integer N, mode parameter, N = M, M + 1, M + 2, ...
!
!    Input, real ( kind = rk8 ) C, spheroidal parameter.
!
!    Input, real ( kind = rk8 ) CV, the characteristic value.
!
!    Input, real ( kind = rk8 ) QT, ?
!
!    Input, real ( kind = rk8 ) CK(*), ?
!
!    Output, real ( kind = rk8 ) BK(*), the coefficients.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) bk(200)
  real ( kind = rk8 ) c
  real ( kind = rk8 ) ck(200)
  real ( kind = rk8 ) cv
  real ( kind = rk8 ) eps
...
!*****************************************************************************80
!
!! cchg() computes the Kummer confluent hypergeometric function.
!
!  Discussion:
!
!    This function computes the Kummer confluent hypergeometric function
...
!  Input:
!
!    real ( kind = rk8 ) A, B, parameter values.
!
...
!
  ...
  if ( real ( z, kind = rk8 ) < 0.0D+00 ) then
    z1 = - z
  end if
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, Y, the real and imaginary parts of
!    the argument Z.
!
!    Input, integer KF, the function code.
!    0 for ln[(z)]
!    1 for (z)
!
!    Output, real ( kind = rk8 ) GR, GI, the real and imaginary parts of
...
!
  ...
  real ( kind = rk8 ), save, dimension ( 10 ) :: a = (/ &
...
!*****************************************************************************80
!
!! chgm() computes the Kummer confluent hypergeometric function M(a,b,x).
!
!  Licensing:
!
!    This routine is copyrighted by Shanjie Zhang and Jianming Jin.  However,
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) A, B, parameters.
!
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) HG, the value of M(a,b,x).
!
  implicit none
...
!*****************************************************************************80
!
!! chgu() computes the Tricomi confluent hypergeometric function U(a,b,x).
!
!  Licensing:
!
!    This routine is copyrighted by Shanjie Zhang and Jianming Jin.  However,
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) HU, U(a,b,x).
!
...
!*****************************************************************************80
!
!! chgubi(): Tricomi confluent hypergeometric function with integer argument B.
!
!  Discussion:
!
!    This procedure computes the Tricomi confluent hypergeometric function
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) HU, the value of U(a,b,x).
!
...
!*****************************************************************************80
!
!! chguit() computes the Tricomi hypergeometric function using Gauss-Legendre integration.
!
!  Discussion:
!
!    This procedure computes the Tricomi hypergeometric function U(a,b,x) by
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) HU, U(a,b,z).
!
...
!
  ...
  integer m
  real ( kind = rk8 ) s
  real ( kind = rk8 ), save, dimension ( 30 ) :: t = (/ &
  ...
  real ( kind = rk8 ) t3
  real ( kind = rk8 ) t4
  real ( kind = rk8 ), save, dimension ( 30 ) :: w = (/ &
...
!*****************************************************************************80
!
!! chgul(): Tricomi confluent hypergeometric function U(a,b,x) for large argument X.
!
!  Licensing:
!
!    This routine is copyrighted by Shanjie Zhang and Jianming Jin.  However,
...
!*****************************************************************************80
!
!! chgus(): Tricomi confluent hypergeometric function U(a,b,x) for small argument X.
!
!  Licensing:
!
!    This routine is copyrighted by Shanjie Zhang and Jianming Jin.  However,
...
!
  ...
  real ( kind = rk8 ), save, dimension ( 12 ) :: a = (/ &
  ...
  real ( kind = rk8 ) a0
  real ( kind = rk8 ), save, dimension ( 10 ) :: a1 = (/ &
  ...
  real ( kind = rk8 ), save, dimension ( 12 ) :: b = (/ &
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) V, the order of Iv(z) and Kv(z).
!
!    Input, complex ( kind = ck ) Z, the argument.
!
!    Output, real ( kind = rk8 ) CBIV, CDIV, CBKV, CDKV, the values of
!    Iv(z), Iv'(z), Kv(z), Kv'(z).
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) V, the order of the functions.
!
!    Input, complex ( kind = ck ) Z, the argument.
!
!    Output, real ( kind = rk8 ) VM, the highest order computed.
!
!    Output, real ( kind = rk8 ) CBI(0:N), CDI(0:N), CBK(0:N), CDK(0:N),
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, the argument of Ci(x) and Si(x).
!
!    Output, real ( kind = rk8 ) CI, SI, the values of Ci(x) and Si(x).
!
  implicit none
...
!  Input:
!
!    real ( kind = rk8 ) X, the argument of Ci(x) and Si(x).
!
!  Output:
!
!    real ( kind = rk8 ) CI, SI, the values of Ci(x) and Si(x).
!
  implicit none
...
!
!    Output, real ( kind = rk8 ) A(L), the value of Cj(k) where j and k are
...
!
  ...
  real ( kind = rk8 ) a0
  real ( kind = rk8 ), save, dimension ( 12 ) :: a1 = (/ &
  ...
  real ( kind = rk8 ), save, dimension ( 12 ) :: b1 = (/ &
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) V, the order of Jv(z) and Yv(z).
!
!    Input, complex ( kind = ck ) Z, the argument.
!
...
!    Output, real ( kind = rk8 ) VM, the highest order computed.
!
!    Output, real ( kind = rk8 ) CBJ(0:*), CDJ(0:*), CBY(0:*), CDY(0:*),
...
!
!    Input, real ( kind = rk8 ) X, Y, the real and imaginary parts
!    of the argument.
!
...
!
!    Input, real ( kind = rk8 ) X, Y, the real and imaginary parts of the
!    argument Z.
!
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) HK, the modulus.  0 <= HK <= 1.
!
!    Output, real ( kind = rk8 ) CK, CE, the values of K(HK) and E(HK).
!
  implicit none
...
!    of the argument.
!
!    Output, real ( kind = rk8 ) PSR, PSI, the real and imaginary parts
...
!
  ...
  real ( kind = rk8 ), save, dimension ( 8 ) :: a = (/ &
...
!
!    Input, real ( kind = rk8 ) Q, the parameter of the functions.
!
!    Output, real ( kind = rk8 ) A0, the characteristic value.
!
  implicit none
...
!
!    Input, real ( kind = rk8 ) Q, the parameter of the Mathieu functions.
!
!    Output, real ( kind = rk8 ) CV(*), characteristic values.
...
!    Input, real ( kind = rk8 ) Q, the parameter of the Mathieu functions.
!
!    Output, real ( kind = rk8 ) A, the characteristic value.
!
  implicit none
  ...
  if ( m <= 12 .or. q <= 3.0D+00 * m .or. m * m < q ) then
    ...
    ndiv = 10
    delta = ( m - 3.0D+00 ) * m / real ( ndiv, kind = rk8 )
...
!    Input, real ( kind = rk8 ) Q, the parameter of the Mathieu functions.
!
!    Input, real ( kind = rk8 ) A, the characteristic value.
!
!    Input, integer MJ, ?
!
!    Output, real ( kind = rk8 ) F, the value of the function for the
...
!    Input, integer M, the order of the Mathieu functions.
!
!    Input, real ( kind = rk8 ) Q, the parameter value.
!
!    Output, real ( kind = rk8 ) A0, the initial characteristic value.
!
  implicit none
...
!    1, for real roots.
!
!    Output, real ( kind = rk8 ) ZO(NT), ZV(NT), the zeros of Y0(z) or Y1(z)
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Input, real ( kind = rk8 ) VA, the order.
!
!    Output, real ( kind = rk8 ) PD, the function value.
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) E1, the function value.
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) EI, the function value.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) HK, the modulus, between 0 and 1.
!
!    Input, real ( kind = rk8 ) PHI, the argument in degrees.
!
!    Output, real ( kind = rk8 ) FE, EE, the values of F(k,phi) and E(k,phi).
!
  implicit none
...
!    Input, real ( kind = rk8 ) HK, the modulus, between 0 and 1.
!
!    Input, real ( kind = rk8 ) C, the parameter, between 0 and 1.
!
!    Output, real ( kind = rk8 ) EL3, the value of the elliptic integral
!    of the third kind.
!
  implicit none
  ...
  logical lb2
  real ( kind = rk8 ) phi
  real ( kind = rk8 ), dimension ( 10 ), save :: t = (/ &
  ...
  real ( kind = rk8 ) t1
  real ( kind = rk8 ) t2
  real ( kind = rk8 ), dimension ( 10 ), save :: w = (/ &
...
!
!    Input, real ( kind = rk8 ) X, the absolute value of the argument.
!
!    Output, real ( kind = rk8 ) ENVJ, the value.
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) EN(0:N), the function values.
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) ERR, the function value.
!
  implicit none
...
!
!    Output, real ( kind = rk8 ) EN(0:N), the Euler numbers up to the N-th value.
!
  implicit none
...
!    Input, real ( kind = rk8 ) Q, the parameter of the Mathieu functions.
!
!    Input, real ( kind = rk8 ) A, the characteristic value of the Mathieu
!    functions for given m and q.
!
!    Output, real ( kind = rk8 ) FC(*), the expansion coefficients of Mathieu
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) C, S, the function values.
!
  implicit none
...
!
  ...
  do nr = 1, nt
    ...
    if ( kf == 1 ) then
      ...
      psq = 2.0D+00 * sqrt ( real ( nr, kind = rk8 ) )
    end if
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) FR, FI, FM, FA, the values of
!    Re[F+/-(x)], Im[F+/-(x)], |F+/-(x)|, Arg[F+/-(x)]  (Degs.).
!
!    Output, real ( kind = rk8 ) GR, GI, GM, GA, the values of
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) GA, the function value.
!
  implicit none
...
!
!    Output, real ( kind = rk8 ) GA, the value of the Gamma function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ), dimension ( 26 ) :: g = (/ &
    1.0D+00, &
    0.5772156649015329D+00, &
   -0.6558780715202538D+00, &
   -0.420026350340952D-01, &
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Input, real ( kind = rk8 ) BK(*), coefficients.
!
!    Output, real ( kind = rk8 ) GF, GD, the value of Gmn(-C,X) and Gmn'(-C,X).
!
  implicit none
...
!
!    Output, real ( kind = rk8 ) X(N), the zeros.
!
!    Output, real ( kind = rk8 ) W(N), the corresponding weights.
!
  implicit none
  ...
  hn = 1.0D+00 / n
  zl = -1.1611D+00 + 1.46D+00 * sqrt ( real ( n, kind = rk8 ) )
...
!*****************************************************************************80
!
!! hygfx() evaluates the hypergeometric function 2F1(A,B,C;X).
!
!  Licensing:
!
!    The original FORTRAN77 version of this routine is copyrighted by
...
!  Input:
!
!    real ( kind = rk8 ) A, B, C, X, the arguments of the function.
...
!
  ...
  real ( kind = rk8 ) c0
  real ( kind = rk8 ) c1
  real ( kind = rk8 ), parameter :: el = 0.5772156649015329D+00
  real ( kind = rk8 ) eps
  real ( kind = rk8 ) f0
  real ( kind = rk8 ) f1
  real ( kind = rk8 ) g0
      ...
      if ( 0 <= m ) then
        ...
        do k = 1, m
          sp0 = sp0 + 1.0D+00 / ( a + k - 1.0D+00 ) &
            + 1.0D+00 / ( b + k - 1.0D+00 ) - 1.0D+00 / real ( k, kind = rk8 )
        end do
        ...
        do k = 1, m
          sp0 = sp0 + 1.0D+00 / real ( k, kind = rk8 )
        end do
        ...
        do k = 1, 250
          ...
          sm = 0.0D+00
          do j = 1, m
            sm = sm + 1.0D+00 / real ( j + k, kind = rk8 )
          end do
...
!*****************************************************************************80
!
!! hygfz() computes the hypergeometric function 2F1(a,b,c;z) for complex argument.
!
!  Licensing:
!
!    This routine is copyrighted by Shanjie Zhang and Jianming Jin.  However,
...
!  Input:
!
!    real ( kind = rk8 ) A, B, C, parameters.
!
!    complex ( kind = ck ) Z, the argument.
!
!  Output:
!
!    complex ( kind = ck ) ZHF, the value of 2F1(a,b,c;z).
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) BI0, DI0, BI1, DI1, BK0, DK0, BK1, DK1, the
...
!
  ...
  real ( kind = rk8 ), save, dimension ( 12 ) :: a = (/ &
    ...
    2.4380529699556D+01, 1.1001714026925D+02, &
    5.5133589612202D+02, 3.0380905109224D+03 /)
  real ( kind = rk8 ), save, dimension ( 8 ) :: a1 = (/ &
    0.125D+00, 0.2109375D+00, &
    1.0986328125D+00, 1.1775970458984D+01, &
...
!    Output, integer NM, the highest order computed.
!
!    Output, real ( kind = rk8 ) BI(0:N), DI(0:N), BK(0:N), DK(0:N),
...
!
  ...
  real ( kind = rk8 ) bi(0:n)
  real ( kind = rk8 ) bi0
  real ( kind = rk8 ) bi1
  real ( kind = rk8 ) bk(0:n)
  real ( kind = rk8 ) bk0
  real ( kind = rk8 ) bk1
  real ( kind = rk8 ) di(0:n)
  real ( kind = rk8 ) di0
  real ( kind = rk8 ) di1
  real ( kind = rk8 ) dk(0:n)
  real ( kind = rk8 ) dk0
  real ( kind = rk8 ) dk1
  real ( kind = rk8 ) f
  real ( kind = rk8 ) f0
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) V, the order of Iv(x) and Kv(x).
!    V = N + V0.
!
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) VM, the highest order computed.
!
!    Output, real ( kind = rk8 ) BI(0:N), DI(0:N), BK(0:N), DK(0:N), the
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) BIX, the function value.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) A, the parameter.
!
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) GIN, GIM, GIP, the values of
!    r(a,x), (a,x), P(a,x).
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, the upper limit of the integral.
!
!    Output, real ( kind = rk8 ) APT, BPT, ANT, BNT, the integrals, from 0 to x,
...
!
  ...
  real ( kind = rk8 ), save, dimension ( 16 ) :: a = (/ &
...
!    Input, real ( kind = rk8 ) X, the upper limit of the integral.
!
!    Output, real ( kind = rk8 ) TI, TK, the integrals of I0(t) and K0(t)
!    from 0 to X.
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the upper limit of the integral.
!
!    Output, real ( kind = rk8 ) TI, TK, the integral of I0(t) and K0(t)
!    from 0 to X.
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the upper limit of the integral.
!
!    Output, real ( kind = rk8 ) TJ, TY, the integrals of J0(t) and Y0(t)
!    from 0 to x.
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the upper limit of the integral.
!
!    Output, real ( kind = rk8 ) TH0, the integral of H0(t) from 0 to x.
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the upper limit of the integral.
!
!    Output, real ( kind = rk8 ) TL0, the integral of L0(t) from 0 to x.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, the lower limit of the integral.
!
!    Output, real ( kind = rk8 ) TTH, the integral of H0(t)/t from x to oo.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, the integral limit.
!
!    Output, real ( kind = rk8 ) TTI, TTK, the integrals of [I0(t)-1]/t
...
!
  ...
  real ( kind = rk8 ) b1
  real ( kind = rk8 ), save, dimension ( 8 ) :: c = (/ &
...
!    Input, real ( kind = rk8 ) X, the integral limit.
!
!    Output, real ( kind = rk8 ) TTI, TTK, the integrals of
...
!    Input, real ( kind = rk8 ) X, the integral limit.
!
!    Output, real ( kind = rk8 ) TTJ, TTY, the integrals of [1-J0(t)]/t
...
!
!    Output, real ( kind = rk8 ) ZO(*), the zeros of Jn(x) and Jn'(x).
!
  implicit none
  ...
  real ( kind = rk8 ) bj(101)
  real ( kind = rk8 ) dj(101)
  real ( kind = rk8 ) fj(101)
  integer i
  integer j
  integer k
  integer l
  ...
  real ( kind = rk8 ) xm
  real ( kind = rk8 ) zo(nt)
  real ( kind = rk8 ) zoc(70)

  if ( nt < 600 ) then
    xm = -1.0D+00 + 2.248485D+00 * real ( nt, kind = rk8 ) ** 0.5D+00 &
      - 0.0159382D+00 * nt + 3.208775D-04 * real ( nt, kind = rk8 ) ** 1.5D+00
    nm = int ( 14.5D+00 + 0.05875D+00 * nt )
    mm = int ( 0.02D+00 * nt ) + 6
  else
    xm = 5.0D+00 + 1.445389D+00 * ( real ( nt, kind = rk8 ) ) ** 0.5D+00 &
      + 0.01889876D+00 * nt &
      - 2.147763D-04 * ( real ( nt, kind = rk8 ) ) ** 1.5D+00
  ...
  do i = 1,nm
    ...
    x1 = 0.407658D+00 + 0.4795504D+00 &
      * ( real ( i - 1, kind = rk8 ) ) ** 0.5D+00 + 0.983618D+00 * ( i - 1 )
    x2 = 1.99535D+00 + 0.8333883 * ( real ( i - 1, kind = rk8 ) ) ** 0.5D+00 &
      + 0.984584D+00 * ( i - 1 )
    l1 = 0
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) U, the argument.
!
!    Input, real ( kind = rk8 ) HK, the modulus, between 0 and 1.
!
!    Output, real ( kind = rk8 ) ESN, ECN, EDN, EPH, the values of
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) BJ0, DJ0, BJ1, DJ1, BY0, DY0, BY1, DY1,
...
!    Output, integer NM, the highest order computed.
!
!    Output, real ( kind = rk8 ) BJ(0:N), DJ(0:N), BY(0:N), DY(0:N), the values
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) BJN, DJN, FJN, BYN, DYN, FYN, the values of
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) V, the order of Jv(x) and Yv(x).
!
!    Input, real ( kind = rk8 ) X, the argument of Jv(x) and Yv(x).
!
!    Output, real ( kind = rk8 ) VM, the highest order computed.
!
!    Output, real ( kind = rk8 ) BJ(0:N), DJ(0:N), BY(0:N), DY(0:N),
...
!    Input, integer NT, the number of zeros.
!
!    Output, real ( kind = rk8 ) RJ0(NT), RJ1(NT), RY0(NT), RY1(NT), the zeros
!    of Jn(x), Jn'(x), Yn(x), Yn'(x).
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) BER, BEI, GER, GEI, DER, DEI, HER, HEI,
...
!    8 for kei' x.
!
!    Output, real ( kind = rk8 ) ZO(NT), the zeros of the given Kelvin function.
!
  implicit none
...
!    -1, the oblate function.
!
!    Input, real ( kind = rk8 ) DF(*), the expansion coefficients.
!
  implicit none
  ...
  real ( kind = rk8 ) r4
  real ( kind = rk8 ) r5
  real ( kind = rk8 ) rv(200)
  real ( kind = rk8 ) sa0
  real ( kind = rk8 ) sb0
  real ( kind = rk8 ) su0
  real ( kind = rk8 ) sw
  real ( kind = rk8 ) t
  real ( kind = rk8 ) tp(200)
  real ( kind = rk8 ) u(200)
  real ( kind = rk8 ) v(200)
  real ( kind = rk8 ) w(200)
...
!
!    Output, real ( kind = rk8 ) X(N), the zeros of the Laguerre polynomial.
!
!    Output, real ( kind = rk8 ) W(N), the weighting coefficients.
!
  implicit none
...
!    Output, integer NM, the highest order computed.
!
!    Output, real ( kind = rk8 ) BL(0:N), DL(0:N), the
...
!
  ...
  real ( kind = rk8 ) bg
  real ( kind = rk8 ) bk
  real ( kind = rk8 ) bl(0:n)
  real ( kind = rk8 ) bs
  real ( kind = rk8 ) dl(0:n)
  real ( kind = rk8 ) f
  real ( kind = rk8 ) f0
  real ( kind = rk8 ) f1
  integer i
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) V, the order.
!
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) VM, the highest order computed.
!
!    Output, real ( kind = rk8 ) VL(0:*), DL(0:*), the Lambda function and
...
!    Input, integer N, the order of the polynomial.
!
!    Output, real ( kind = rk8 ) X(N), W(N), the zeros of the polynomial,
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) GL, the function value.
!
  implicit none
...
!
!    Input, real ( kind = rk8 ) X, the argument of Pmn(x).
!
!    Output, real ( kind = rk8 ) PM(0:MM,0:N), PD(0:MM,0:N), the
...
!
  ...
  integer ls
  integer m
  real ( kind = rk8 ) pd(0:mm,0:n)
  real ( kind = rk8 ) pm(0:mm,0:n)
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xq
  real ( kind = rk8 ) xs
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) PM(0:N), PD(0:N), the values and derivatives
...
!
  ...
  integer k
  integer m
  real ( kind = rk8 ) pm(0:n)
  real ( kind = rk8 ) pm0
  real ( kind = rk8 ) pm1
  real ( kind = rk8 ) pm2
  real ( kind = rk8 ) pmk
  real ( kind = rk8 ) pd(0:n)
  real ( kind = rk8 ) x
  real ( kind = rk8 ) x0
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) V, the degree of Pmv(x).
!
!    Input, integer M, the order of Pmv(x).
!
!    Input, real ( kind = rk8 ) X, the argument of Pm(x).
!
!    Output, real ( kind = rk8 ) PMV, the value of Pm(x).
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) PN(0:N), PD(0:N), the values and derivatives
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) PN(0:N), PD(0:N), PL(0:N), the values,
...
!
!    Output, real ( kind = rk8 ) QM(0:MM,0:N), QD(0:MM,0:N), contains the values
!    of Qmn(x) and Qmn'(x).
!
  implicit none
  ...
  real ( kind = rk8 ) q1
  real ( kind = rk8 ) q10
  real ( kind = rk8 ) qd(0:mm,0:n)
  real ( kind = rk8 ) qf
  real ( kind = rk8 ) qf0
  real ( kind = rk8 ) qf1
  real ( kind = rk8 ) qf2
  real ( kind = rk8 ) qm(0:mm,0:n)
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xq
  real ( kind = rk8 ) xs
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) QM(0:N), QD(0:N), the values of Qmn(x)
!    and Qmn'(x).
!
  implicit none
...
!
!    Input, real ( kind = rk8 ) X, the argument of Qn(x).
!
!    Output, real ( kind = rk8 ) QN(0:N), QD(0:N), the values of
!    Qn(x) and Qn'(x).
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, the argument of Jn(x).
!
...
!    Input, real ( kind = rk8 ) Q, the parameter of the Mathieu functions.
!
!    Input, real ( kind = rk8 ) X, the argument of the Mathieu functions,
!    in degrees.
!
!    Output, real ( kind = rk8 ) CSF, CSD, the values of cem(x,q) and cem'(x,q),
...
!    Input, real ( kind = rk8 ) X, the argument of the Mathieu functions.
!
!    Output, real ( kind = rk8 ) F1R, D1R, F2R, D2R, the values of
...
!
  ...
  real ( kind = rk8 ) a
  real ( kind = rk8 ) bj1(0:251)
  real ( kind = rk8 ) bj2(0:251)
  real ( kind = rk8 ) by1(0:251)
  real ( kind = rk8 ) by2(0:251)
  real ( kind = rk8 ) c1
  real ( kind = rk8 ) c2
  real ( kind = rk8 ) d1r
  real ( kind = rk8 ) d2r
  real ( kind = rk8 ) dj1(0:251)
  real ( kind = rk8 ) dj2(0:251)
  real ( kind = rk8 ) dy1(0:251)
  real ( kind = rk8 ) dy2(0:251)
  real ( kind = rk8 ) eps
  real ( kind = rk8 ) f1r
  real ( kind = rk8 ) f2r
  real ( kind = rk8 ) fg(251)
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) PL(0:N), DPL(0:N), the value and derivative of
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) DV(0:*), DP(0:*), the values of
!    Dn+v0(x), Dn+v0'(x).
!
!    Output, real ( kind = rk8 ) PDF, PDD, the values of Dv(x) and Dv'(x).
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) VV(0:*), VP(0:*), the values of Vv(x), Vv'(x).
!
!    Output, real ( kind = rk8 ) PVF, PVD, the values of Vv(x) and Vv'(x).
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) W1F, W1D, W2F, W2D, the values of
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) PS, the value of the PSI function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ), parameter :: a1 = -0.83333333333333333D-01
  real ( kind = rk8 ), parameter :: a2 =  0.83333333333333333D-02
  real ( kind = rk8 ), parameter :: a3 = -0.39682539682539683D-02
  real ( kind = rk8 ), parameter :: a4 =  0.41666666666666667D-02
  real ( kind = rk8 ), parameter :: a5 = -0.75757575757575758D-02
  real ( kind = rk8 ), parameter :: a6 =  0.21092796092796093D-01
  real ( kind = rk8 ), parameter :: a7 = -0.83333333333333333D-01
  real ( kind = rk8 ), parameter :: a8 =  0.4432598039215686D+00
  real ( kind = rk8 ), parameter :: el = 0.5772156649015329D+00
  integer k
  integer n
  real ( kind = rk8 ), parameter :: pi = 3.141592653589793D+00
  ...
  if ( x == aint ( x ) .and. x <= 0.0D+00 ) then
    ...
    do k = 1, n
      s = s + 1.0D+00 / real ( 2 * k - 1, kind = rk8 )
    end do
    ...
    if ( xa < 10.0D+00 ) then
      ...
      n = 10 - int ( xa )
      do k = 0, n - 1
        s = s + 1.0D+00 / ( xa + real ( k, kind = rk8 ) )
      end do

      xa = xa + real ( n, kind = rk8 )
...
!    Input, real ( kind = rk8 ) CK(*), ?
!
!    Input, real ( kind = rk8 ) CK1, ?
!
!    Output, real ( kind = rk8 ) QS, ?
!
!    Output, real ( kind = rk8 ) QT, ?
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) ap(200)
  real ( kind = rk8 ) c
  real ( kind = rk8 ) ck(200)
  real ( kind = rk8 ) ck1
  integer i
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, the argument of the function.
!
!    Output, real ( kind = rk8 ) R8_GAMMA_LOG, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ), dimension ( 7 ) :: c = (/ &
    -1.910444077728D-03, &
     8.4171387781295D-04, &
    ...
    -2.777777777777681622553D-03, &
     ...
     5.7083835261D-03 /)
  real ( kind = rk8 ) corr
  real ( kind = rk8 ) :: d1 = -5.772156649015328605195174D-01
  real ( kind = rk8 ) :: d2 = 4.227843350984671393993777D-01
  real ( kind = rk8 ) :: d4 = 1.791759469228055000094023D+00
  real ( kind = rk8 ), parameter :: frtbig = 2.25D+76
  integer i
  real ( kind = rk8 ), dimension ( 8 ) :: p1 = (/ &
  ...
  real ( kind = rk8 ) r8_gamma_log
  real ( kind = rk8 ) res
  real ( kind = rk8 ), parameter :: sqrtpi = 0.9189385332046727417803297D+00
  real ( kind = rk8 ) x
  real ( kind = rk8 ), parameter :: xbig = 2.55D+305
  real ( kind = rk8 ) xden
  real ( kind = rk8 ), parameter :: xinf = 1.79D+308
  real ( kind = rk8 ) xm1
  real ( kind = rk8 ) xm2
  real ( kind = rk8 ) xm4
  real ( kind = rk8 ) xnum
...
!    Output, integer NM, the highest order computed.
!
!    Output, real ( kind = rk8 ) RJ(0:N), the values of x jn(x).
!
!    Output, real ( kind = rk8 ) DJ(0:N), the values of [x jn(x)]'.
!
  implicit none
...
!    Output, integer NM, the highest order computed.
!
!    Output, real ( kind = rk8 ) RY(0:N), the values of x yn(x).
!
!    Output, real ( kind = rk8 ) DY(0:N), the values of [x yn(x)]'.
!
  implicit none
...
!    Input, real ( kind = rk8 ) Q, the parameter of the Mathieu functions.
!
!    Input/output, real ( kind = rk8 ) A, the characteristic value, which
...
!    -1, the oblate function.
!
!    Output, real ( kind = rk8 ) R1F, R1D, the function and derivative.
!
  implicit none
...
!    -1, the oblate function.
!
!    Output, real ( kind = rk8 ) R2F, R2D, the function and derivative values.
!
  implicit none
...
!    -1, the oblate function.
!
!    Output, real ( kind = rk8 ) R2F, R2D, the values of Rmn(-ic,ix)
!    and Rmn'(-ic,ix).
!
  implicit none
...
!    -1, the oblate function.
!
!    Output, real ( kind = rk8 ) R2F, R2D, the values of the function and
!    its derivative.
!
  implicit none
  ...
  real ( kind = rk8 ) pd(0:251)
  real ( kind = rk8 ) pm(0:251)
  real ( kind = rk8 ) qd(0:251)
  real ( kind = rk8 ) qm(0:251)
  real ( kind = rk8 ) r1
  real ( kind = rk8 ) r2
  real ( kind = rk8 ) r2d
  real ( kind = rk8 ) r2f
...
!
!    Output, real ( kind = rk8 ) R1F, the radial function of the first kind;
!
!    Output, real ( kind = rk8 ) R1D, the derivative of the radial function of
!    the first kind;
!
!    Output, real ( kind = rk8 ) R2F, the radial function of the second kind;
!
!    Output, real ( kind = rk8 ) R2D, the derivative of the radial function of
!    the second kind;
!
  implicit none
...
!    Input, real ( kind = rk8 ) C, spheroidal parameter.
!
!    Input, real ( kind = rk8 ) X, the argument of the radial function, 1 < X.
!
!    Input, real ( kind = rk8 ) CV, the characteristic value.
!
!    Input, integer KF, the function code.
...
!    -1, the oblate function.
!
!    Output, real ( kind = rk8 ) CK(*), the expansion coefficients.
...
!    Input, real ( kind = rk8 ) C, the spheroidal parameter.
!
!    Input, real ( kind = rk8 ) DF(*), the expansion coefficients DK.
!
!    Output, real ( kind = rk8 ) CK(*), the expansion coefficients CK.
!
  implicit none
...
!    -1, the oblate function.
!
!    Output, real ( kind = rk8 ) DF(*), expansion coefficients;
...
!    -1, the oblate function.
!
!    Output, real ( kind = rk8 ) CV, the characteristic value.
!
!    Output, real ( kind = rk8 ) EG(*), the characteristic value for
...
!    Output, integer NM, the highest order computed.
!
!    Output, real ( kind = rk8 ) SI(0:N), DI(0:N), the values and derivatives
...
!    Output, integer NM, the highest order computed.
!
!    Output, real ( kind = rk8 ) SJ(0:N), the values of jn(x).
!
!    Output, real ( kind = rk8 ) DJ(0:N), the values of jn'(x).
!
  implicit none
...
!
    ...
    if ( abs ( x ) <= 1.0D-16 ) then
      ...
      if ( 0 < n ) then
        do k = 1, n
          sj(k) = sj(k-1) * x / real ( 2 * k + 1, kind = rk8 )
        end do
...
!    Output, integer NM, the highest order computed.
!
!    Output, real ( kind = rk8 ) SK(0:N), DK(0:N), the values of kn(x) and kn'(x).
!
  implicit none
...
!    Output, integer NM, the highest order computed.
!
!    Output, real ( kind = rk8 ) SY(0:N), DY(0:N), the values of yn(x) and yn'(x).
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) SH0, the value of H0(x).
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) SH1, the value of H1(x).
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) V, the order of the function.
!
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) HV, the value of Hv(x).
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) SL0, the function value.
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Output, real ( kind = rk8 ) SL1, the function value.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) V, the order of Lv(x).
!
!    Input, real ( kind = rk8 ) X, the argument of Lv(x).
!
!    Output, real ( kind = rk8 ) SLV, the value of Lv(x).
!
  implicit none
...
!    Input, real ( kind = rk8 ) X, the argument.
!
!    Input, real ( kind = rk8 ) VA, the order nu.
!
!    Output, real ( kind = rk8 ) PV, the value of V(nu,x).
!
  implicit none
